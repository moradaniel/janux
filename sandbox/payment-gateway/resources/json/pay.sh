...
#curl -L -k -u hera:hera https://localhost/payment-jnx/app/creditCard.json -G -d `cat $1`
curl -L -k -u user1:password1 https://localhost/payment/app/creditCard.json -G -d `cat $1`
