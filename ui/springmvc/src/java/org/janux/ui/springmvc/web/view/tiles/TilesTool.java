...
/**
 * <p>Abstract tool class for combining Tiles and your favorite view technology</p>
 * <p><pre>
 * Template example(s):
 *  &lt;!-- insert a tile --&gt;
 ...
 */
public abstract class TilesTool 
{
	 ...
	 */
	public TilesTool(HttpServletRequest request, HttpServletResponse response, ApplicationContext applicationContext)
	{
	...
	public String get(Object obj) throws Exception
	{
		...
		{
			throw e;
//			logger.error("TilesTool: Exeption while rendering Tile " + obj + ": " + e.getMessage());
//			return null;
		}
	}
	...
	public Object getAttribute(String name) {
		...
		if (value == null) {
			logger.warn("TilesTool: Tile attribute '"	+ name + "' was not found in context.");
		}
		return value;
	}
	...
	protected abstract void putAttribute(String name, Object value);
	...
	public void importAttributes(String scope) {
		...
		if (scope.equals(PAGE_SCOPE)) {
			while (names.hasNext()) {
				...
				this.putAttribute(name, context.getAttribute(name));
			}
		} else if (scope.equals(REQUEST_SCOPE)) {
			while (names.hasNext()) {
				String name = (String) names.next();
	 ...
	 * @throws Exception
	 */
	protected String doInsert(ComponentContext subCompContext, String page, String role, 
			TileController controller) throws Exception
	{
		pushTilesContext();
		try {
			...
			if (controller != null) {
				...
				if(model != null) {
				    for (Iterator it = model.keySet().iterator(); it.hasNext(); ) {
                        ...
                        this.putAttribute(key, model.get(key));
                    }
				}
			}
	...
	{
		String result = this.doRender(page);
			
		if (result == null)
		{
			result = new ImportSupport(application, request, response).acquireString(page);
		}
		
		return result;
	}

	protected abstract String doRender(String page) throws Exception;
